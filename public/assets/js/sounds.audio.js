...
$(document).ready(function () {
  function initiate() {
    ...
    const prevNextElement = $(".previous_next_container");
    const descriptionContainer = $(".description_container");
    const newPrevNext = prevNextElement[0];

    if (windowWidth <= 360) {
      $(".audio-controller > div:not(.center)").addClass("hidden");
      prevNextElement.addClass("hidden");
      $(".btn_switch_desc").removeClass("hidden");
      $(".btn_switch_desc > button#desc_btn").addClass("active");
      $(".description_episode_inner > .episode_inner").addClass("hidden");
      descriptionContainer.append(newPrevNext);
      prevNextElement.removeClass("hidden");

      if ($(".recommend-link").length % 2 != 0) {
        $(".recommend-link:last-of-type").addClass("hidden");
      }
    } else {
      $(".audio-controller > div:not(.center)").removeClass("hidden");
      $(".btn_switch_desc").addClass("hidden");
      prevNextElement.addClass("hidden");
      prevNextElement.insertBefore(".description_episode_inner");
      prevNextElement.removeClass("hidden");
      $(".description_episode_inner > div").removeClass("hidden");

      $(".recommend-link").removeClass("hidden");
    }
  }
  ...
  $(".btn_switch_desc > button").on("mouseup", function (ev) {
    $(".btn_switch_desc > button").removeClass("active");
    this.classList.add("active");

    if (this.id == "eps_btn") {
      $(".description_episode_inner").removeClass("hidden");
      $(".description_episode_inner > div").addClass("hidden");
      $(".description_episode_inner > .episode_inner").removeClass("hidden");
    } else if (this.id == "desc_btn") {
      $(".description_episode_inner").removeClass("hidden");
      $(".description_episode_inner > div").addClass("hidden");
      $(".description_episode_inner > .description_inner").removeClass("hidden");
    }
  });
  ...
  function volumeLineControl(target) {
    ...
    $(".audio-controller > .right > #volume-controller > .volume-bar").click(function () {
      let count = 0;
      ...
      backIntervalBtn.addEventListener("mouseup", (ev) => {
        ...
        setTimeout(() => {
          ...
          if (!$("audio")[0].isPlayed) {
            ...
            $(".duration-notation > #currentTime")[0].parentNode.classList.remove("hidden");
          }
        }, 500);
  ...
  function setBarProgress() {
    let progress = ($("audio")[0].currentTime / $("audio")[0].duration) * 100;
  ...
  function addBarProgresEv() {
    ...
    $(".p-bar.no-progress").on("mousemove", (ev) => {
      let progress = ev.originalEvent.layerX / ev.delegateTarget.offsetWidth + 0.0012422361;
      let left = (ev.originalEvent.layerX / ev.delegateTarget.offsetWidth) * 100 + 0.01;
  ...
  function onTimeUpdate() {
    ...
    // jika end audio
    if ($("audio")[0].ended && $("#playBtn > *").filter(".active")[0].dataset.playerState == "pause") {
      togglePlayPause();
  ...
  function addTogglePlayPause() {
    ...
    $("#playBtn").click(() => {
      ...
      playerState = $("#playBtn > *").filter(".hidden")[0].dataset.playerState;
  ...
  volumeLineControl($(".audio-controller > .right > #volume-controller > .volume-bar > .volume-line"));
