...
function volumeLineControl(target) {
    ...
    $(".audio-controller > .right > #volume-controller > .volume-bar").click(
        () => {
            let count = 0;
...
function addBarProgresEv() {
    ...
    $(".p-bar.no-progress").on("mousemove", (ev) => {
        let progress =
            ...
            0.0012422361;
        let left =
            (ev.originalEvent.layerX / ev.delegateTarget.offsetWidth) * 100 +
            0.01;
...
function onTimeUpdate() {
    ...
    if (
        $("audio")[0].ended &&
        $("#playBtn > *").filter(".active")[0].dataset.playerState == "pause"
    ) {
        togglePlayPause();
